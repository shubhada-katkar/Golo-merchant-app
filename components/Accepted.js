...
const AcceptedCard = memo(function AcceptedCard({ order, onComplete, colors }) {
  ...
  return (
    <View style={styles.card2}>
      ...
      <View style={{
        ...
        <Text style={{
          ...textPresets.caption, color: "#5f5f5f", lineHeight: Math.round(14 * 1.5)
        }}
...
export default function Accepted({ orders = [], onComplete }) {
  ...
  const handleComplete = useCallback((order) => {
    onComplete?.(order);
  }, [onComplete]);

  const renderItem = useCallback(({ item }) => (
    <AcceptedCard order={item} onComplete={handleComplete} colors={colors} />
  ), [handleComplete, colors]);
  ...
  return (
    <FlatList
      ...
      ListEmptyComponent={<Text style={{ textAlign: "center", marginTop: 24 }}>No accepted orders</Text>}
