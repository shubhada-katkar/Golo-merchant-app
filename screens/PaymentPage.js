...
export default function PaymentPage({ navigation, route }) {
  ...
  const handlePayWithRazorpay = async () => {
    ...
    try {
      ...
      const totalAmountInRupees = Math.round(totalAmount);
      ...
      let orderData = null;
      let errorMessage = "";
      const idempotencyKey = `pay_sub_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
  ...
  const getRazorpayCheckoutHtml = () => {
    ...
    const optionsObj = {
      key: keyId,
      amount: order?.amount ? Number(order.amount) : Math.round(totalAmount * 100),
