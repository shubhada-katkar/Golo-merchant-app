...
function ClaimedOfferContent() {
  ...
  useEffect(() => {
    ...
    // If we already have the voucher in context and it has verification data, use it.
    // Otherwise fetch from API to ensure QR/verificationCode are populated.
    if (selectedVoucherMatches && selectedVoucher?.verificationCode && selectedVoucher?.qrImage) {
      setVoucherAccessRestricted(false);
      return;
    }

    // Otherwise fetch from API (handles page reload or incomplete voucher state)
  ...
  }, [voucherId, user, selectedVoucher, fetchVoucherDetails]);
