...
function buildLegacyPromotionPayload(payload = {}) {
    ...
    if (loyaltyPointsPerPurchase !== undefined && mapped.loyaltyStarsPerPurchase === undefined && mapped.loyaltyScorePerStar === undefined) {
        mapped.loyaltyStarsToOffer = 1;
        mapped.loyaltyStarsPerPurchase = 1;
        mapped.loyaltyScorePerStar = Number(loyaltyPointsPerPurchase || 1);
    }
    delete mapped.loyaltyPointsPerPurchase;
...
export async function submitOfferPromotionRequest(payload) {
    const enrichedPayload = {
        ...payload,
        bannerTitle: payload.bannerTitle || payload.title,
        bannerCategory: payload.bannerCategory || payload.category,
        promotionType: 'offer',
    };

    if (enrichedPayload.title !== undefined) {
        delete enrichedPayload.title;
    }
    if (enrichedPayload.category !== undefined) {
        delete enrichedPayload.category;
    }

    if (enrichedPayload.loyaltyPointsPerPurchase !== undefined) {
        const points = Number(enrichedPayload.loyaltyPointsPerPurchase || 1);
        enrichedPayload.loyaltyStarsToOffer = enrichedPayload.loyaltyStarsToOffer ?? 1;
        enrichedPayload.loyaltyStarsPerPurchase = enrichedPayload.loyaltyStarsPerPurchase ?? 1;
        enrichedPayload.loyaltyScorePerStar = enrichedPayload.loyaltyScorePerStar ?? points;
        delete enrichedPayload.loyaltyPointsPerPurchase;
    }
...
export async function updateMyOfferPromotion(promotionId, updateData) {
    const response = await apiClient(`/banners/promotions/${promotionId}`, {
        method: 'PUT',
        body: JSON.stringify(updateData),
    });
